...
def find_nearest_unblocked(grid, position):
    ...
    return None  # No unblocked cells found
...
def visualize_grid_with_path(grid, path, filename):
    ...
    grid_img = np.zeros((*grid.shape, 3))  # Create RGB image
...
def runAll50Grids():
    ...
    Logs and compares performance (nodes expanded).
    Saves an image of the grid with each algorithm's path.
    """
    grid_dir = "gridworlds/"
    ...
    for i in range(1, 51):  # Change back to (1, 51) to run all 50 grids
